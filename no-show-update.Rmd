...
no_show_audit <- no_show_audit %>%
  mutate(numerator = ifelse(APPT_STATUS_NAME == "No Show" | (APPT_STATUS_NAME == "Canceled" & CANCEL_LEAD_HOURS <=24 & CANCEL_INITIATOR == "PATIENT"), 1, 0),
         denominator = ifelse(APPT_STATUS_NAME %in% c("Arrived", "Checked In", "Checked Out", "Completed", "Left without seen") |  (APPT_STATUS_NAME == "Canceled" & CANCEL_LEAD_HOURS <24 & CANCEL_INITIATOR == "PATIENT"), 1, 0))


no_show_audit_temp_tbl <- tbl(conn2, "Y_NOSHOW_AUDIT_TEMP")
no_show_audit_temp <- no_show_audit_temp_tbl %>%
  collect()


require(openxlsx)
write.xlsx(no_show_audit, "No Show Logic Validation Mar25.xlsx")
...
no_show_mshs_baseline_physician <- access_raw_tbl %>%
  ...
  mutate(perc_noShowPlus = (`No Show` + Late_Canceled)/(Completed + `Left without seen` + `No Show` + Late_Canceled),
         perc_noShow = (`No Show`)/(Completed + `Left without seen` + `No Show`)) %>%
  rename("Completed_Physician" = Completed,
         "No_Show_Physician" = `No Show`,
         "Left_without_seen_Physician" = `Left without seen`,
         "Late_Canceled_Physician" = Late_Canceled,
...
payer_mix_dep <- access_raw_tbl %>%
  ...
  filter(CONTACT_DATE < TO_DATE(date_start, "YYYY-MM-DD HH24:MI:SS")) %>%
  filter(appt_status %in% c("Arrived")) %>%
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, appt_year, appt_month, new_pt_los, FINCLASS, VISITPLAN) %>%
  summarise(total = n()) %>%
  collect() 
  # pivot_wider(names_from = new_pt_los,
  #             values_from = total,
...
# require(openxlsx)
# write.xlsx(payer_mix_dep, "EPIC Payer Mix_032625.xlsx")
```
...
ref_dept <- caboodle_referrals_conn %>%
  filter(ReceivedByDept %LIKE% "%BREAST%") %>%
  ...
  collect() %>%
    mutate(exclude = ifelse(ProcedureName %in% ref_exclude, "Y", "N")) 
...
#          start_month = month(Creation_Date)) %>%
#   mutate(TicketsSentToPatBy_X = ifelse(TicketsSentToPatBy_X=="", "None", TicketsSentToPatBy_X)) %>%
#   group_by(ReceivedBySite, ReceivedByDept, ReceivedByDepID, ReceivedByProvNPI, ReceivedByProvider, start_year, start_month, TicketsSentToPatBy_X) %>%
#   summarise(non_converted = n_distinct(ReferralEpicId[is.na(AppointmentStatus)]),
#          scheduled = n_distinct(ReferralEpicId[!is.na(AppointmentStatus)]),
#          completed = n_distinct(ReferralEpicId[AppointmentStatus %in% c("Arrived", "Completed")])) %>%
#   collect() %>%
...
referrals_dep_check <- department_merged %>%
  group_by(DEPARTMENT_ID, Year, Month) %>%
  summarise(total_referrals = sum(total_referrals, na.rm = T)) %>%
  left_join(referrals_dep %>% group_by(DEPARTMENT_ID, Year, Month) %>% summarise(total_referrals_org = sum(total_referrals, na.rm = T))) %>%
  mutate(check = total_referrals - total_referrals_org)
...
mshs_roi <- mshs_roi %>%
  mutate(Completed_Physician_var_baseline = (Completed_Physician - Completed_Physician_prior_year)/Completed_Physician_prior_year,
         prov_util_var_baseline= (prov_util - prov_util_prior_year),
         prov_util_actualized= ifelse(prov_util_var_baseline >0, prov_util_var_baseline*Completed_Physician, 0),
         ...
         perc_noShowPlus_Physician_actualized= ifelse(perc_noShowPlus_Physician_var_baseline >0, 0, 
                                                      perc_noShowPlus_Physician_var_baseline*(Completed_Physician + No_Show_Physician + Late_Canceled_Physician)*(-1)),
...
col_order <- c("Month", "Year",
               "Completed_Physician", "Completed_Physician_prior_year", "Completed_Physician_var_baseline",
               "avail_hours_prior_year", "avail_hours", "arrived_hours_prior_year", "arrived_hours", "prov_util_prior_year", "prov_util",
               ...
               "No_Show_Physician", "Late_Canceled_Physician",
...
reactable(
  ...
  columnGroups = list(
    ...
    colGroup(name = paste0("Volume Completed by Physicians*"),
             columns = c("Completed_Physician", "Completed_Physician_prior_year", "Completed_Physician_var_baseline"),
    ...
    colGroup(name = paste0("C.1.1 - C.1.3 Estimated Impact: Provider Utilization*"),
             columns = c("avail_hours_prior_year", "avail_hours", "arrived_hours", "arrived_hours_prior_year", "prov_util_prior_year", "prov_util",
                         "prov_util_var_baseline", "prov_util_actualized"),
    ...
    colGroup(name = paste0("C.1.1 No Show & Late Cancel Reduction*"),
             columns = c("perc_noShowPlus_Physician_prior_year", "perc_noShowPlus_Physician",
                         "perc_noShowPlus_Physician_var_baseline", "No_Show_Physician", "Late_Canceled_Physician",
                         "perc_noShowPlus_Physician_actualized", "perc_noShowPlus_Physician_actualized_dollar",
  ...
  columns = list(
    ...
    Completed_Physician_prior_year = colDef(
      name = "2024 Baseline",
      maxWidth = 100,
      headerStyle = list(background = "#7f7f7f", color = "white", fontWeight = "Bold", fontSize = "14px"),
      # Calculate the aggregate conversion rate as `(total-pending) / total`
    ...
    Completed_Physician_var_baseline = colDef(
    ...
    arrived_hours_prior_year = colDef(
    ...
    prov_util_actualized = colDef(
      ...
      #     totalArrived_Physician += row['Arrived_Physician']
      #   })
      #   return Math.max(0,((totalarrived_hours/totalavail_hours) - (totalarrived_hours_prior_year/totalavail_hours_prior_year))*totalArrived_Physician) || '-'
      # }"),
      format = colFormat(separators = TRUE, digits = 0)
      ),
    ...
    perc_noShowPlus_Physician_prior_year = colDef(
      ...
      aggregate = JS("function(values, rows) {
        let totalCompleted_Physician_prior_year = 0
        let totalLeft_without_seen_Physician_prior_year = 0
        let totalNo_Show_Physician_prior_year = 0
        let totalLate_Canceled_Physician_prior_year = 0
        rows.forEach(function(row) {
          totalCompleted_Physician_prior_year += row['Completed_Physician_prior_year']
          totalLeft_without_seen_Physician_prior_year += row['Left_without_seen_Physician_prior_year']
          totalNo_Show_Physician_prior_year += row['No_Show_Physician_prior_year']
          totalLate_Canceled_Physician_prior_year += row['Late_Canceled_Physician_prior_year']
        })
        return (totalNo_Show_Physician_prior_year + totalLate_Canceled_Physician_prior_year) / (totalCompleted_Physician_prior_year + totalLeft_without_seen_Physician_prior_year + totalNo_Show_Physician_prior_year + totalLate_Canceled_Physician_prior_year) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 1)),
    ...
    perc_noShowPlus_Physician = colDef(
      ...
      headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totalCompleted_Physician = 0
        let totalLeft_without_seen_Physician = 0
        let totalNo_Show_Physician = 0
        let totalLate_Canceled_Physician = 0
        rows.forEach(function(row) {
          totalCompleted_Physician += row['Completed_Physician']
          totalLeft_without_seen_Physician += row['Left_without_seen_Physician']
          totalNo_Show_Physician += row['No_Show_Physician']
          totalLate_Canceled_Physician += row['Late_Canceled_Physician']
        })
        return (totalNo_Show_Physician + totalLate_Canceled_Physician) / (totalCompleted_Physician + totalLeft_without_seen_Physician + totalNo_Show_Physician + totalLate_Canceled_Physician) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 1)),
    ...
    perc_noShowPlus_Physician_var_baseline = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return ((totalNo_Show_Physician + totalLate_Canceled_Physician) / (totalCompleted_Physician + totalLeft_without_seen_Physician + totalNo_Show_Physician + totalLate_Canceled_Physician)) - ((totalNo_Show_Physician_prior_year + totalLate_Canceled_Physician_prior_year) / (totalCompleted_Physician_prior_year + totalLeft_without_seen_Physician_prior_year + totalNo_Show_Physician_prior_year + totalLate_Canceled_Physician_prior_year)) || '-'
      }"),
      style = JS("function(rowInfo) {
    ...
    Late_Canceled_Physician = colDef(
    ...
    perc_noShowPlus_Physician_var_target = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return ((totalNo_Show_Physician + totalLate_Canceled_Physician) / (totalCompleted_Physician + totalLeft_without_seen_Physician + totalNo_Show_Physician + totalLate_Canceled_Physician))-.12 || '-'
      }"),
    style = JS("function(rowInfo) {
    ...
    Late_Cancel_Physician_prior_year = colDef(show =F),
...
reactable(
  ...
  columnGroups = list(
    ...
    colGroup(name = paste0("No Show"),
             columns = c("perc_noShow", "perc_noShowPlus"),
             headerStyle = list(background = "#d80b8c", color = "white", fontSize = "15px")),
  ...
  columns = list(
    ...
    perc_noShow = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        let totalLeft_without_seen = 0
        let totalNo_Show = 0
        rows.forEach(function(row) {
          totalCompleted += row['Completed']
          totalLeft_without_seen += row['Left_without_seen']
          totalNo_Show += row['No_Show']
        })
        return (totalNo_Show) / (totalCompleted + totalLeft_without_seen + totalNo_Show) || '-'
      }"),
      style = JS("function(rowInfo) {
    ...
    perc_noShowPlus = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        let totalLeft_without_seen = 0
        let totalNo_Show = 0
        let totalLate_Canceled = 0
        rows.forEach(function(row) {
          totalCompleted += row['Completed']
          totalLeft_without_seen += row['Left_without_seen']
          totalNo_Show += row['No_Show']
          totalLate_Canceled += row['Late_Canceled']
        })
        return (totalNo_Show + totalLate_Canceled) / (totalCompleted + totalLeft_without_seen + totalNo_Show + totalLate_Canceled) || '-'
      }"),
      style = JS("function(rowInfo) {
    ...
    sched_hours = colDef(show =F),
    arrived_hours = colDef(show =F),
    Completed = colDef(show =F),
    Left_without_seen = colDef(show =F),
    Late_Canceled = colDef(show =F),
    No_Show = colDef(show =F),
    perc_SameDayCanceled = colDef(show =F),
    NEW_LOS = colDef(show =F),
    ...
    Completed_Physician = colDef(show =F),
    Left_without_seen_Physician = colDef(show =F),
    No_Show_Physician = colDef(show =F),
    Late_Canceled_Physician = colDef(show =F),
