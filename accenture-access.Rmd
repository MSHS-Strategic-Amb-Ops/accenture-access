...
slot_util <- slot_view_tbl %>%
  ...
  mutate(AVAILABLE_MINS_ADJ = max(AVAILABLE_MINS, SCHEDULED_MINS)) %>%
  collect() %>%
  left_join(clarity_ser) %>%
  left_join(clarity_ser_2)

slot_util_check <- slot_util %>%
  filter(PROV_TYPE == "Physician") %>%
  mutate(sched_util = SCHEDULED_MINS/AVAILABLE_MINS)
...
slot_util_mshs_baseline <- slot_util %>%
  ...
  summarise(avail_hours = sum(AVAILABLE_MINS, na.rm = T)/60,
            avail_hours_adj = sum(AVAILABLE_MINS_ADJ, na.rm = T)/60,
  ...
  mutate(sched_util = sched_hours/avail_hours,
         ...
         sched_util_adj = sched_hours/avail_hours_adj,
         prov_util_adj = arrived_hours/avail_hours_adj) %>%
...
mshs_roi <- mshs_roi %>%
  mutate(prov_util_var_baseline= (prov_util - prov_util_prior_year),
         ...
         prov_util_actualized_budget= (prov_util - prov_util_prior_year)*(avail_hours)*110,
...
reactable(
  ...
  columns = list(
    ...
    prov_util_var_baseline = colDef(
      name = "Variance to 2024 Baseline",
      maxWidth = 100,
      headerStyle = list(background = "#212070", color = "white", fontWeight = "Bold", fontSize = "14px"),
      # Calculate the aggregate conversion rate as `(total-pending) / total`
      aggregate = JS("function(values, rows) {
...
reactable(
  ...
  columnGroups = list(
    ...
    colGroup(name = paste0("Template Utilization (Physicians Only)"),
             columns = c("avail_hours", "sched_util", "prov_util",
                         "avail_hours_adj", "sched_util_adj", "prov_util_adj"),
             headerStyle = list(background = "#212070", color = "white", fontSize = "15px")),
  ...
  columns = list(
    ...
    avail_hours_adj = colDef(
      name = "Available Hrs Adj.",
      aggregate = "sum",
      maxWidth = 100,
      headerStyle = list(background = "#212070", color = "white", fontWeight = "Bold", fontSize = "14px"),
      format = colFormat(separators = TRUE, digits = 0)),

    sched_util_adj = colDef(
      name = "Schedule Utilization Adj.",
      maxWidth = 100,
      headerStyle = list(background = "#212070", color = "white", fontWeight = "Bold", fontSize = "14px"),
      # Calculate the aggregate conversion rate as `(total-pending) / total`
      aggregate = JS("function(values, rows) {
        let totalsched_hours = 0
        let totalavail_hours_adj = 0
        rows.forEach(function(row) {
          totalsched_hours += row['sched_hours']
          totalavail_hours_adj += row['avail_hours_adj']
        })
        return (totalsched_hours) / (totalavail_hours_adj) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0)
    ),

    prov_util_adj = colDef(
      name = "Provider Utilization Adj.",
      maxWidth = 100,
      headerStyle = list(background = "#212070", color = "white", fontWeight = "Bold", fontSize = "14px"),
      # Calculate the aggregate conversion rate as `(total-pending) / total`
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_hours) / (totalavail_hours_adj) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0)
    ),
