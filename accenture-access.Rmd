...
clarity_ser <- clarity_ser_tbl %>% head(10) %>% collect()
clarity_ser_2 <- clarity_ser_2_tbl %>% head(10) %>% collect()
...
no_show_mshs_baseline_current <- access_raw_tbl %>%
  # left_join(clarity_ser_tbl %>% select(PROV_ID, PROV_TYPE)) %>%
  # filter(PROV_TYPE == "Physician") %>%
...
#       onclick = "Reactable.downloadDataCSV('access-roid', 'Access Metrics ROI')"
#     ),
