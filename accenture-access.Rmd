...
slot_util_mshs_baseline <- slot_merged %>%
  ...
  summarise(avail_min = sum(avail_min, na.rm = T),
            arrived_min = sum(arrived_min, na.rm = T)) %>%
  mutate(prov_util = arrived_min/avail_min) %>%
  collect() %>%
...
#   left_join(f_sched_appt_tbl %>%
#               dplyr::select(PAT_ENC_CSN_ID, COULD_DIR_SCHED_C, COULD_OPEN_SCHED_C, COULD_TKT_SCHED_C, APPT_SCHED_SOURCE_C, PAT_ONLINE_YN)) %>%
#   group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, appt_made_year, appt_made_month, PROV_NAME_WID,
...
# 
# 
# appt_avail_online_mshs_baseline_physician <- appt_online_physician %>%
#   filter(appt_made_year == prior_year) %>%
#   filter(avail_online_exclude == "N") %>%
#   group_by(appt_made_year, appt_made_month, avail_online, sched_online) %>%
#   summarise(appts_sched = sum(total, na.rm = T))  %>%
...
# 
# 
# appt_avail_online_mshs_ytd_physician <- appt_online_physician %>%
#   filter(appt_made_year == current_year) %>%
#   filter(avail_online_exclude == "N") %>%
#   group_by(appt_made_year, appt_made_month, avail_online, sched_online) %>%
#   summarise(appts_sched = sum(total, na.rm = T))  %>%
...
# 
# 
# appt_avail_online_dep_physician <- appt_online_physician %>%
#   filter(avail_online_exclude == "N") %>%
#   group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, appt_made_year, appt_made_month, avail_online, sched_online) %>%
#   summarise(appts_sched = sum(total, na.rm = T))  %>%
#   pivot_wider(names_from = c("avail_online", "sched_online"),
...
col_order <- c("Month", "Year",
               ...
               "avail_min_prior_year", "avail_min", "arrived_min_prior_year", "arrived_min", "prov_util_prior_year", "prov_util",
...
reactable(
  ...
  columnGroups = list(
    ...
    colGroup(name = paste0("C.1.1 - C.1.3 Estimated Impact: Provider Utilization*"),
             columns = c("avail_min_prior_year", "avail_min", "arrived_min", "arrived_min_prior_year", "prov_util_prior_year", "prov_util",
                         "prov_util_var_baseline", "prov_util_actualized"),
  ...
  columns = list(
    ...
    #   # Calculate the aggregate conversion rate as `(total-pending) / total`
    #   aggregate = JS("function(values, rows) {
    #     let totalavail_min_prior_year = 0
    #     let totalsched_hours_prior_year = 0
    #     rows.forEach(function(row) {
    #       totalavail_min_prior_year += row['avail_min_prior_year']
    #       totalsched_hours_prior_year += row['sched_hours_prior_year']
    #     })
    #     return (totalsched_hours_prior_year) / (totalavail_min_prior_year) || '-'
    #   }"),
    #   format = colFormat(percent = TRUE, digits = 1)
    # ),
    # 
    ...
    #     })
    #     return (totalsched_hours) / (totalavail_min) || '-'
    #   }"),
    #   format = colFormat(percent = TRUE, digits = 1)
    # ),
    ...
    prov_util_prior_year = colDef(
      ...
      aggregate = JS("function(values, rows) {
        let totalavail_min_prior_year = 0
        let totalarrived_min_prior_year = 0
        rows.forEach(function(row) {
          totalavail_min_prior_year += row['avail_min_prior_year']
          totalarrived_min_prior_year += row['arrived_min_prior_year']
        })
        return (totalarrived_min_prior_year) / (totalavail_min_prior_year) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 1)
    ),
    ...
    prov_util_var_baseline = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_min/totalavail_min) - (totalarrived_min_prior_year/totalavail_min_prior_year) || '-'
      }"),
      style = JS("function(rowInfo) {
    ...
    prov_util_actualized = colDef(
      ...
      #     totalArrived_Physician += row['Arrived_Physician']
      #   })
      #   return Math.max(0,((totalarrived_min/totalavail_min) - (totalarrived_min_prior_year/totalavail_min_prior_year))*totalArrived_Physician) || '-'
      # }"),
      format = colFormat(separators = TRUE, digits = 0)
      ),
    ...
    avail_min_prior_year = colDef(show =F),
    sched_hours_prior_year = colDef(show =F),
    arrived_min_prior_year = colDef(show =F),
    ...
    avail_min_adj = colDef(show =F),
    sched_util_adj = colDef(show =F),
    prov_util_adj = colDef(show =F),
    avail_min_adj_prior_year = colDef(show =F),
