...
slot_data_tbl <- tbl(conn1, "SLOT_DATA1")
...
slot_util_mshs_baseline <- slot_merged %>%
  filter(!is.na(SITE)) %>%
  filter(SITE %in% c("MSUS", "MSH- AMBULATORY CARE", "NETWORK", "MSW", "MSH-MSDFP", 
                     "MSDMG", "MSB", "MSM", "MSQ", "MSBI", "MSSN", "NYEE")) %>%
  ...
  group_by(DEPT_SPECIALTY_NAME, DEPARTMENT_ID, DEPARTMENT_NAME, PROV_ID, PROV_NM_WID, slot_date) %>%
  summarise(avail_min = sum(REG_AVAIL_MIN, na.rm = T),
            arrived_min = sum(ARRIVED_MIN, na.rm = T)) %>%
  mutate(prov_util = case_when(avail_min == 0 | is.na(avail_min) ~ 0, TRUE ~ arrived_min/avail_min)) %>%
  ...
  summarise(avail_hour = sum(avail_min, na.rm = T)/60,
            arrived_hour = sum(arrived_min, na.rm = T)/60) %>%
  mutate(prov_util = arrived_hour/avail_hour) %>%
  collect() %>%
  rename("Year" = slot_year,
         "Month" = slot_month)


slot_util_mshs_ytd <- slot_merged %>%
  filter(!is.na(SITE)) %>%
  filter(SITE %in% c("MSUS", "MSH- AMBULATORY CARE", "NETWORK", "MSW", "MSH-MSDFP", 
                     "MSDMG", "MSB", "MSM", "MSQ", "MSBI", "MSSN", "NYEE")) %>%
  mutate(SPECIALTY_GRP = ifelse(DEPT_SPECIALTY_NAME %in% 
...
slot_util_dep <- slot_merged %>%
  ...
  mutate(SPECIALTY_GRP = ifelse(DEPT_SPECIALTY_NAME %in% 
                                  c("Adolescent Medicine", "Community and Preventive Medicine", 
                                    "Family Medicine", "Internal Medicine", "Pediatrics", "Geriatrics", "Gerontology", "Urgent Care"), "Primary Care", "Specialty")) %>%
  mutate(slot_date = sql("TRUNC(SLOT_BEGIN_TIME)")) %>%
  group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_ID, DEPARTMENT_NAME, PROV_ID, PROV_NM_WID, slot_date) %>%
  summarise(avail_min = sum(REG_AVAIL_MIN, na.rm = T),
            arrived_min = sum(ARRIVED_MIN, na.rm = T)) %>%
  mutate(prov_util = case_when(avail_min == 0 | is.na(avail_min) ~ 0, TRUE ~ arrived_min/avail_min)) %>%
  filter(prov_util >=0.35 & prov_util <=2.0) %>%
  mutate(slot_year = year(slot_date),
         slot_month = month(slot_date)) %>%
  filter(year(slot_date) >= prior_year) %>%
  group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_ID, DEPARTMENT_NAME, slot_year, slot_month) %>%
  summarise(avail_hour = sum(avail_min, na.rm = T)/60,
            arrived_hour = sum(arrived_min, na.rm = T)/60) %>%
  mutate(prov_util = arrived_hour/avail_hour) %>%
  collect() %>%
...
payer_mix_dep <- access_raw_tbl %>%
  ...
  filter(CONTACT_DATE < TO_DATE(date_start, "YYYY-MM-DD HH24:MI:SS")) %>%
  filter(appt_status %in% c("Arrived")) %>%
  mutate(FINCLASS = case_when(FINCLASS %in% c("Commercial/Managed Care") ~ "COMMERCIAL_MANAGED_CARE",
...
# require(openxlsx)
# write.xlsx(payer_mix_dep2, "EPIC Payer Mix_070825.xlsx")
```
...
# require(openxlsx)
# write.xlsx(department_merged, "department_merged_080625.xlsx")
...
```
<br/>
<p style="font-size:10pt; font-style:italic">*Volume, Provider Utilization, and No Show metrics shown on this page are based on physician visits only (PROV_TYPE = "Physician").<p/>
