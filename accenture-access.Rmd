...
#   mutate(appt_status = ifelse(DERIVED_STATUS_DESC == "Canceled" & lead_days <=0, "SameDay_Canceled", DERIVED_STATUS_DESC)) %>%
...
slot_util <- slot_view_tbl %>%
  ...
  mutate(SPECIALTY_GRP = ifelse(CAMPUS_SPECIALTY %in% c("Family Medicine", "Internal Medicine"), "Primary Care", "Specialty")) %>%
  group_by(CAMPUS, SPECIALTY_GRP, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROVIDER, PROV_ID, SLOT_DATE, slot_year, slot_month) %>%
...
# slot_util_prov <- slot_view_tbl %>%
#   mutate(slot_year = year(SLOT_DATE),
#          slot_month = month(SLOT_DATE)) %>%
#   filter(slot_year >= prior_year) %>%
#   group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROV_ID, PROVIDER, slot_year, slot_month) %>%
#   summarise(avail_hours = sum(AVAILABLE_MINS, na.rm = T)/60,
#             sched_hours = sum(SCHEDULED_MINS, na.rm = T)/60,
#             arrived_hours = sum(ARRIVED_MINS, na.rm = T)/60) %>%
#   group_by(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, DEPARTMENT_ID, PROV_ID, PROVIDER, slot_year, slot_month) %>%
...
no_show_dep <- access_raw_tbl %>%
  ...
  mutate(visit_type = ifelse(is.na(VISIT_GROUP_NUM), "ESTABLISHED", "NEW")) %>%
  mutate(lead_14days = ifelse(lead_days <= 14, "lead_14days", "lead_14days_more")) %>%
   mutate(SPECIALTY_GRP = ifelse(DEPT_SPECIALTY_NAME %in% c("Family Medicine", "Internal Medicine"), "Primary Care", "Specialty")) %>%
...
# no_show_prov <- access_raw %>%
#   filter(year(CONTACT_DATE) >= prior_year) %>%
#   filter(appt_status %in% c("No Show","SameDay_Canceled","Arrived")) %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NAME_WID, PROV_ID, appt_year, appt_month, appt_status) %>%
#   summarise(total = n()) %>%
#   collect() %>%
#   pivot_wider(names_from = appt_status,
#               values_from = total,
...
# require(openxlsx)
# write.xlsx(department_merged %>% filter(Year == 2023), "department_merged_2023.xlsx")
```




```{r ROI Overview, echo=FALSE, out.width = '15%'}

# Target 2025
access_target <- data.frame(
  Month = c(rep(seq(1:12),2)),
  metric_name = c(rep("perc_noShowPlus",12), rep("perc_conversion",12)),
  accenture_target_low = c(rep(0.10,12), rep(0.61,12)),
  accenture_target_high = c(rep(0.07,12), rep(0.63,12)),
  mshs_target = c(rep(0.12,12), rep(0.60,12)),
  mshs_impact = c(rep(2000000/12,12), rep(1000000/12,12))
)
...
mshs_roi <- mshs_roi %>%
  mutate(noShow_actual = ifelse(perc_noShowPlus < perc_noShowPlus_prior_year, (perc_noShowPlus_prior_year-perc_noShowPlus)*(Arrived + SameDay_Canceled + No_Show)*110, 0),
         noShow_budget_actualized = (round(noShow_actual/perc_noShowPlus_mshs_impact,2)),
         conversion_actual = ifelse(perc_conversion > perc_conversion_prior_year, (perc_conversion-perc_conversion_prior_year)*(total_referrals)*110, 0), 
         conversion_budget_actualized = (round(conversion_actual/perc_conversion_mshs_impact,2))) %>%
...
# Accenture-Access Workstream Metrics & Impact {.tabset .tabset-fade .tabset-pills}
...
reactable(
  ...
  columnGroups = list(
    ...
    colGroup(name = paste0("C.1.1 No Show & Same-Day Cancel Reduction"),
             columns = c("perc_noShowPlus_prior_year", "perc_noShowPlus_mshs_target", "perc_noShowPlus_mshs_impact", 
                         "perc_noShowPlus", "noShow_actual", "noShow_budget_actualized"),
             headerStyle = list(background = "#212070", color = "white", fontSize = "15px")),
    # colGroup(name = paste0("Referral Volume"),
    #          columns = c("total_referrals", "total_referrals_prior_year"),
    #          headerStyle = list(background = "#d80b8c", color = "white", fontSize = "15px")),
    colGroup(name = paste0("C.1.3 Referral Retention"),
             columns = c("perc_conversion_prior_year", "perc_conversion_mshs_target", "perc_conversion_mshs_impact",
                          "perc_conversion", "conversion_actual", "conversion_budget_actualized"),
             headerStyle = list(background = "#d80b8c", color = "white", fontSize = "15px"))
    ...
    #          columns = c("total_referrals", "perc_conversion", "perc_completion",
  ...
  columns = list(
    ...
    perc_noShowPlus_mshs_impact = colDef(
    ...
    noShow_budget_actualized = colDef(
      name = "2025 Target Impact Actualized",
      maxWidth = 100,
      headerStyle = list(background = "#212070", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totalnoShow_actual = 0
        let totalperc_noShowPlus_mshs_impact = 0
        rows.forEach(function(row) {
          totalnoShow_actual += row['noShow_actual']
          totalperc_noShowPlus_mshs_impact += row['perc_noShowPlus_mshs_impact']
        })
        return (totalnoShow_actual) / (totalperc_noShowPlus_mshs_impact) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 1)),
