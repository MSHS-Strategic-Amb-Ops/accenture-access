...
ref_baseline <- caboodle_referrals_conn %>%
  ...
  group_by(ReferralEpicId, start_year, start_month) %>%
  summarise(baseline = "Y") %>%
  collect()
...
ref_dept <- caboodle_referrals_conn %>%
  ...
  mutate(start_year = year(Creation_Date),
         start_month = month(Creation_Date)) %>%
  group_by(ReceivedByDept, ReceivedByDepID, start_year, start_month, TicketsSentToPatBy_X, ReferralEpicId) %>%
  summarise(dept = "Y") %>%
  collect()
