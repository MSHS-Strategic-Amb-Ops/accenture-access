...
slot_data_raw_tbl <- tbl(conn1, "ALL_SLOTS_2023")
slot_data_tbl <- tbl(conn1, "SLOT_DATA_2023")
...
prior_year <- as.character(as.numeric(current_year) -3)
...
grp_mapping <- data.frame("SITE" = c("NETWORK", "OTHER", "MSH-MSDFP", "MSUS", 
                                        "MSDMG", "MSBI", "MSW", "MSHS", "MSM", "MSH- AMBULATORY CARE",
...
slot_util_mshs_ytd <- slot_merged_physician %>%
  ...
  mutate(slot_year = year(SLOT_BEGIN_TIME),
         slot_month = month(SLOT_BEGIN_TIME)) %>%
  filter(year(slot_date) == current_year) %>%
  group_by(slot_year, slot_month, DEPARTMENT_ID, PROV_ID) %>%
  summarise(avail_hour = sum(REG_AVAIL_MIN, na.rm = T)/60,
            sched_hour = sum(SCHED_MIN, na.rm = T)/60,
            arrived_hour = sum(ARRIVED_MIN, na.rm = T)/60) %>%
...
#   filter(PROV_TYPE == "Physician") %>%
#   group_by(slot_year, slot_month) %>%
#   summarise(avail_min = sum(AVAILABLE_MINS, na.rm = T)/60,
#             sched_hours = sum(SCHEDULED_MINS, na.rm = T)/60,
#             arrived_min = sum(ARRIVED_MINS, na.rm = T)/60) %>%
#   mutate(sched_util = sched_hours/avail_min,
#          prov_util = arrived_min/avail_min) %>%
#   rename("Year" = slot_year,
#          "Month" = slot_month)
# 
# 
...
#   filter(PROV_TYPE == "Physician") %>%
#   group_by(CAMPUS, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT, DEPARTMENT_ID, slot_year, slot_month) %>%
#   summarise(avail_min = sum(AVAILABLE_MINS, na.rm = T)/60,
#             sched_hours = sum(SCHEDULED_MINS, na.rm = T)/60,
#             arrived_min = sum(ARRIVED_MINS, na.rm = T)/60) %>%
#   mutate(sched_util = sched_hours/avail_min,
...
#   group_by(CAMPUS, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT, DEPARTMENT_ID, PROV_ID, PROV_TYPE, PROVIDER, slot_year, slot_month) %>%
#   summarise(avail_min = sum(AVAILABLE_MINS, na.rm = T)/60,
#             sched_hours = sum(SCHEDULED_MINS, na.rm = T)/60,
#             arrived_min = sum(ARRIVED_MINS, na.rm = T)/60) %>%
#   mutate(sched_util = sched_hours/avail_min,
...
#   mutate(cancel_lead_days = CONTACT_DATE - APPT_CANC_DATE) %>%
#   # mutate(lead_days =  as.numeric(difftime(as.Date(APPT_MADE_DATE), as.Date(CONTACT_DATE), units = "days"))) %>%
#    filter(!(is.na(APPT_CANC_DTTM) & APPT_STATUS_NAME == "Canceled")) %>%
#   mutate(appt_status = ifelse(DERIVED_STATUS_DESC == "Canceled" & cancel_lead_days <=0, "SameDay_Canceled", DERIVED_STATUS_DESC)) %>%
#  
#   mutate(appt_year = year(CONTACT_DATE),
#          appt_month = month(CONTACT_DATE)) %>%
...
#   group_by(appt_year, appt_month, appt_status, PROV_ID) %>%
#   summarise(total = n()) %>%
#   collect() %>%
#   left_join(clarity_ser) %>%
#   filter(PROV_TYPE == "Physician") %>%
#   group_by(appt_year, appt_month, appt_status, PROV_TYPE) %>%
...
#   # mutate(lead_days =  as.numeric(difftime(as.Date(APPT_MADE_DATE), as.Date(CONTACT_DATE), units = "days"))) %>%
#    filter(!(is.na(APPT_CANC_DTTM) & APPT_STATUS_NAME == "Canceled")) %>%
#   mutate(appt_status = ifelse(APPT_STATUS_NAME %in% c("Arrived", "Checked In", "Checked Out", "Completed"), "Completed", APPT_STATUS_NAME)) %>%
#   mutate(appt_status = ifelse(appt_status == "Canceled" & CANCEL_LEAD_HOURS <24 & CANCEL_INITIATOR == "PATIENT", "Late_Canceled", appt_status)) %>%
#   mutate(appt_year = year(CONTACT_DATE),
#          appt_month = month(CONTACT_DATE)) %>%
#   mutate(appt_made_year = year(APPT_MADE_DATE),
#          appt_made_month = month(APPT_MADE_DATE)) %>%
...
#   filter(year(CONTACT_DATE) == 2025) %>%
#   filter(CONTACT_DATE < TO_DATE(date_start, "YYYY-MM-DD HH24:MI:SS")) %>%
#   filter(appt_status %in% c("No Show","Late_Canceled","Completed","Left without seen")) %>%
#   group_by(appt_year, appt_month, appt_status, PROV_ID) %>%
#   summarise(total = n()) %>%
#   collect() %>%
#   left_join(clarity_ser) %>%
...
#               values_from = total,
#               values_fill = 0) %>%
#   mutate(perc_noShowPlus = (`No Show` + Late_Canceled)/(Completed + `Left without seen` + `No Show` + Late_Canceled),
#          perc_noShow = (`No Show`)/(Completed + `Left without seen` + `No Show`)) %>%
#   rename("No_Show" = `No Show`,
#          "Year" = appt_year,
#          "Month" = appt_month)
# 
# 
# 
# status_comparison <-  access_raw_tbl %>%
#   # left_join(clarity_ser_tbl %>% select(PROV_ID, PROV_TYPE)) %>%
#   # filter(PROV_TYPE == "Physician") %>%
#   filter(!is.na(SITE)) %>%
#   filter(SITE %in% c("MSUS", "MSH- AMBULATORY CARE", "NETWORK", "MSW", "MSH-MSDFP", 
...
#   # mutate(lead_days =  as.numeric(difftime(as.Date(APPT_MADE_DATE), as.Date(CONTACT_DATE), units = "days"))) %>%
#    filter(!(is.na(APPT_CANC_DTTM) & APPT_STATUS_NAME == "Canceled")) %>%
#   mutate(appt_status = ifelse(APPT_STATUS_NAME == "Canceled" & CANCEL_LEAD_HOURS <24 & CANCEL_INITIATOR == "PATIENT", "Late_Canceled", APPT_STATUS_NAME)) %>%
#   mutate(appt_year = year(CONTACT_DATE),
#          appt_month = month(CONTACT_DATE)) %>%
#   mutate(appt_made_year = year(APPT_MADE_DATE),
#          appt_made_month = month(APPT_MADE_DATE)) %>%
...
#   filter(year(CONTACT_DATE) == 2025) %>%
#   filter(CONTACT_DATE < TO_DATE(date_start, "YYYY-MM-DD HH24:MI:SS")) %>%
#   group_by(APPT_STATUS_NAME, DERIVED_STATUS_DESC, PROV_ID) %>%
#   summarise(total = n()) %>%
#   collect() %>%
#   left_join(clarity_ser) %>%
#   filter(PROV_TYPE == "Physician") %>%
#   group_by(APPT_STATUS_NAME, DERIVED_STATUS_DESC , PROV_TYPE) %>%
#   summarise(total = sum(total, na.rm = T))
# 
# 
# cancel_dist <- access_raw_tbl %>%
#   # left_join(clarity_ser_tbl %>% select(PROV_ID, PROV_TYPE)) %>%
#   # filter(PROV_TYPE == "Physician") %>%
#   
#   filter(!is.na(SITE)) %>%
...
#   # mutate(lead_days =  as.numeric(difftime(as.Date(APPT_MADE_DATE), as.Date(CONTACT_DATE), units = "days"))) %>%
#    filter(!(is.na(APPT_CANC_DTTM) & APPT_STATUS_NAME == "Canceled")) %>%
#   mutate(appt_status_new = ifelse(APPT_STATUS_NAME == "Canceled" & CANCEL_LEAD_HOURS <24 & CANCEL_INITIATOR == "PATIENT", "Late_Canceled", APPT_STATUS_NAME)) %>%
#   mutate(appt_status_old = ifelse(DERIVED_STATUS_DESC == "Canceled" & cancel_lead_days <=0, "SameDay_Canceled", DERIVED_STATUS_DESC)) %>%
# 
#   mutate(appt_year = year(CONTACT_DATE),
#          appt_month = month(CONTACT_DATE)) %>%
#   mutate(appt_made_year = year(APPT_MADE_DATE),
...
#   filter(year(CONTACT_DATE) == 2025) %>%
#   filter(CONTACT_DATE < TO_DATE(date_start, "YYYY-MM-DD HH24:MI:SS")) %>%
#   filter(APPT_STATUS_NAME == "Canceled") %>%
#   filter(CANCEL_LEAD_HOURS <24) %>%
#   group_by(SITE, DEPARTMENT_NAME, DEPARTMENT_ID, appt_year, appt_month, PAT_ENC_CSN_ID, cancel_lead_days, CANCEL_LEAD_HOURS, APPT_DTTM, APPT_CANC_DTTM, CANCEL_INITIATOR, 
#            APPT_STATUS_NAME, DERIVED_STATUS_DESC, appt_status_new, appt_status_old, PROV_ID) %>%
#   summarise(total = n()) %>%
#   collect() %>%
#   left_join(clarity_ser) %>%
#   filter(PROV_TYPE == "Physician") %>%
#   group_by(SITE, DEPARTMENT_NAME, DEPARTMENT_ID, appt_year, appt_month, PAT_ENC_CSN_ID, cancel_lead_days, CANCEL_LEAD_HOURS, APPT_DTTM, APPT_CANC_DTTM, CANCEL_INITIATOR, 
#            APPT_STATUS_NAME, DERIVED_STATUS_DESC, appt_status_new, appt_status_old, PROV_TYPE) %>%
#   summarise(total = sum(total, na.rm = T))
# 
# 
# data_list <- list("No Show Old" = no_show_old,
#                   "No Show New" = no_show_new,
#                   "Status Comparison" = status_comparison,
#                   "Cancellation" = cancel_dist)
# 
# require(openxlsx)
# write.xlsx(data_list, "No Show Change Impact Physicians.xlsx")
...
#   mutate(cancel_lead_days = CONTACT_DATE - APPT_CANC_DATE) %>%
#   # mutate(lead_days =  as.numeric(difftime(as.Date(APPT_MADE_DATE), as.Date(CONTACT_DATE), units = "days"))) %>%
#   mutate(appt_status = ifelse(DERIVED_STATUS_DESC == "Canceled" & cancel_lead_days <=0, "Late_Cancel", DERIVED_STATUS_DESC)) %>%
#   mutate(appt_year = year(CONTACT_DATE),
#          appt_month = month(CONTACT_DATE)) %>%
#   mutate(appt_made_year = year(APPT_MADE_DATE),
#          appt_made_month = month(APPT_MADE_DATE)) %>%
...
# new_pt_scheduled_prov <- access_raw %>%
#   filter(year(APPT_MADE_DATE) >= prior_year) %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NAME_WID, PROV_ID, appt_made_year, appt_made_month, visit_type) %>%
#   summarise(total = n()) %>%
#   collect() %>%
...
#               values_fill = 0) %>%
#   rowwise() %>%
#   mutate(appts_scheduled = sum(NEW, ESTABLISHED, na.rm = T))
# 
# new_pt_waitTime_prov <- access_raw %>%
#   filter(year(APPT_MADE_DATE) >= prior_year) %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NAME_WID, PROV_ID, appt_year, appt_month, lead_14days) %>%
#   summarise(total = n()) %>%
#   collect() %>%
...
#               values_fill = 0) %>%
#   rowwise() %>%
#   mutate(appts_scheduled = sum(lead_14days, lead_14days_more, na.rm = T))
...
payer_mix <- access_raw_tbl %>%
  ...
  group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, appt_year, appt_month, new_pt_los, FINCLASS) %>%
  summarise(total = n()) %>%
  collect() 
...
# require(openxlsx)
# write.xlsx(payer_mix, "Payer Mix_081525.xlsx")
```
...
# cbo_ref <- read_csv(file.choose())
# 
# caboodle_cbo_audit <- cbo_ref %>%
#   filter(`Year of ENTRY_DATE` == 2024) %>%
#   dplyr::select(REFERRAL_ID, ORDER_NAME, SCHED_APPT_FLAG, COMP_APPT_FLAG) %>%
#   mutate(in_Clarity = "Y") %>%
#   left_join(referrals_merged %>% filter(year(Creation_Date) == 2024) %>%
#               dplyr::select(ReferralEpicId, sched_flag, comp_flag, ProcedureName, Class, Status, ProcedureName, ReceivedByDept) %>%
#               mutate(in_Caboodle = "Y") %>%
#               collect(),
#             by = c("REFERRAL_ID" = "ReferralEpicId")) 
#  
# audit <- caboodle_cbo_audit %>% group_by(in_Clarity, in_Caboodle) %>% summarise(total = n())
# 
# missing_caboodle <- caboodle_cbo_audit %>%
#   filter(is.na(in_Caboodle))
...
# appt_sched_online <- appt_online %>%
#   filter(avail_online_exclude == "N") %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, appt_made_year, appt_made_month, sched_online) %>%
#   summarise(appts_sched = sum(total, na.rm = T))  %>%
#   pivot_wider(names_from = sched_online,
#               values_from = appts_sched,
#               values_fill = 0) %>%
#   mutate(perc_sched_online = Y/(Y+N)) %>%
#   rename("avail_online" = Y ,
#          "not_avail_online" = N,
#          "Year" = appt_made_year,
#          "Month" = appt_made_month)
...
require(openxlsx)
write.xlsx(department_merged, "department_merged_081825.xlsx")
...
mshs_roi <- mshs_roi %>%
  mutate(Arrived_Physician_var_baseline = (Arrived_Physician - Arrived_Physician_prior_year)/Arrived_Physician_prior_year,
         prov_util_var_baseline= (prov_util - prov_util_prior_year),
         prov_util_actualized= ifelse(prov_util_var_baseline >0, prov_util_var_baseline*Arrived_Physician, 0),
         perc_noShowPlus_Physician_var_baseline= (perc_noShowPlus_Physician - perc_noShowPlus_Physician_prior_year),
         perc_noShowPlus_Physician_var_target= (perc_noShowPlus_Physician - perc_noShowPlus_mshs_target),
         ...
         perc_conversion_var_baseline = (perc_conversion - perc_conversion_prior_year),
         perc_conversion_var_target = (perc_conversion - perc_conversion_mshs_target),
         ref_converted_var_target = (total_scheduled - ref_converted_mshs_target)
         ) %>%
...
# Order Columns 
col_order <- c("Month", "Year",
               "Arrived_Physician", "Arrived_Physician_prior_year", "Arrived_Physician_var_baseline",
               "avail_hour_prior_year", "avail_hour", "arrived_hour_prior_year", "arrived_hour", "prov_util_prior_year", "prov_util",
               "prov_util_var_baseline", "prov_util_actualized",
               "perc_noShowPlus_Physician_prior_year", "perc_noShowPlus_Physician",
               "perc_noShowPlus_Physician_var_baseline", "perc_noShowPlus_mshs_target", "perc_noShowPlus_Physician_var_target",
               "No_Show_Physician", "Canceled_Physician",
               "perc_noShowPlus_Physician_actualized", "perc_noShowPlus_Physician_actualized_dollar",
               "total_scheduled_prior_year", "total_scheduled", "ref_converted_mshs_target", "ref_converted_var_target",
               "perc_conversion_prior_year", "perc_conversion", "perc_conversion_var_baseline")
...
#   mutate(across(where(is.numeric), ~replace(., is.na(.), 0))) %>%
#   mutate(`Year-Month` = paste0(Year,"-",Month)) %>%
#   pivot_longer(avail_min:perc_tkt_sent_both,
#                names_to = "metric_name",
#                values_to = "value_name") %>%
#   filter(metric_name %in% c("avail_min",))
# 
# 
...
reactable(
  ...
  columnGroups = list(
    ...
    colGroup(name = paste0("Volume Completed by Physicians*"),
             columns = c("Arrived_Physician", "Arrived_Physician_prior_year", "Arrived_Physician_var_baseline"),
    ...
    colGroup(name = paste0("C.1.1 - C.1.3 Estimated Impact: Provider Utilization*"),
             columns = c("avail_hour_prior_year", "avail_hour", "arrived_hour", "arrived_hour_prior_year", "prov_util_prior_year", "prov_util",
                         "prov_util_var_baseline", "prov_util_actualized"),
             headerStyle = list(background = "#212070", color = "white", fontSize = "15px")),
    colGroup(name = paste0("C.1.1 No Show & Late Cancel Reduction*"),
             columns = c("perc_noShowPlus_Physician_prior_year", "perc_noShowPlus_Physician",
                         "perc_noShowPlus_Physician_var_baseline", "No_Show_Physician", "Canceled_Physician",
                         "perc_noShowPlus_Physician_actualized", "perc_noShowPlus_Physician_actualized_dollar",
                         "perc_noShowPlus_mshs_target", "perc_noShowPlus_Physician_var_target"
                         ),
    ...
    colGroup(name = paste0("C.1.3 Referral Retention"),
             columns = c("total_scheduled_prior_year", "total_scheduled", "ref_converted_mshs_target", "ref_converted_var_target",
                         "perc_conversion_prior_year", "perc_conversion", "perc_conversion_var_baseline"),
  ...
  columns = list(
    ...
    Arrived_Physician_prior_year = colDef(
      name = "2024 Baseline",
      maxWidth = 100,
      headerStyle = list(background = "#7f7f7f", color = "white", fontWeight = "Bold", fontSize = "14px"),
      # Calculate the aggregate conversion rate as `(total-pending) / total`
    ...
    Arrived_Physician_var_baseline = colDef(
      name = "Variance to 2024 Volume",
      maxWidth = 100,
      headerStyle = list(background = "#7f7f7f", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totalArrived_Physician_prior_year = 0
        let totalArrived_Physician = 0
        rows.forEach(function(row) {
          totalArrived_Physician_prior_year += row['Arrived_Physician_prior_year']
          totalArrived_Physician += row['Arrived_Physician']
        })
        return (totalArrived_Physician - totalArrived_Physician_prior_year) / (totalArrived_Physician_prior_year) || '-'
      }"),
    ...
    arrived_hour_prior_year = colDef(
      name = "2024 Arrived Hours",
      maxWidth = 100,
      headerStyle = list(background = "#212070", color = "white", fontWeight = "Bold", fontSize = "14px"),
      # Calculate the aggregate conversion rate as `(total-pending) / total`
      aggregate = 'sum',
    ...
    #   # Calculate the aggregate conversion rate as `(total-pending) / total`
    #   aggregate = JS("function(values, rows) {
    #     let totalavail_hour_prior_year = 0
    #     let totalsched_hours_prior_year = 0
    #     rows.forEach(function(row) {
    #       totalavail_hour_prior_year += row['avail_hour_prior_year']
    #       totalsched_hours_prior_year += row['sched_hours_prior_year']
    #     })
    #     return (totalsched_hours_prior_year) / (totalavail_hour_prior_year) || '-'
    #   }"),
    #   format = colFormat(percent = TRUE, digits = 1)
    # ),
    # 
    ...
    prov_util_prior_year = colDef(
      ...
      aggregate = JS("function(values, rows) {
        let totalavail_hour_prior_year = 0
        let totalarrived_hour_prior_year = 0
        rows.forEach(function(row) {
          totalavail_hour_prior_year += row['avail_hour_prior_year']
          totalarrived_hour_prior_year += row['arrived_hour_prior_year']
        })
        return (totalarrived_hour_prior_year) / (totalavail_hour_prior_year) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 1)
    ),
    ...
    prov_util_var_baseline = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalarrived_hour/totalavail_hour) - (totalarrived_hour_prior_year/totalavail_hour_prior_year) || '-'
      }"),
      style = JS("function(rowInfo) {
    ...
    prov_util_actualized = colDef(
      ...
      #     totalArrived_Physician += row['Arrived_Physician']
      #   })
      #   return Math.max(0,((totalarrived_hour/totalavail_hour) - (totalarrived_hour_prior_year/totalavail_hour_prior_year))*totalArrived_Physician) || '-'
      # }"),
      format = colFormat(separators = TRUE, digits = 0)
      ),

    perc_noShowPlus_Physician_prior_year = colDef(
      name = "2024 Baseline",
      maxWidth = 100,
      headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totalArrived_Physician_prior_year = 0
        let totalNo_Show_Physician_prior_year = 0
        let totalCanceled_Physician_prior_year = 0
        rows.forEach(function(row) {
          totalArrived_Physician_prior_year += row['Arrived_Physician_prior_year']
          totalNo_Show_Physician_prior_year += row['No_Show_Physician_prior_year']
          totalCanceled_Physician_prior_year += row['Canceled_Physician_prior_year']
        })
        return (totalNo_Show_Physician_prior_year + totalCanceled_Physician_prior_year) / (totalArrived_Physician_prior_year + totalNo_Show_Physician_prior_year + totalCanceled_Physician_prior_year) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 1)),
    ...
    perc_noShowPlus_Physician_var_baseline = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return ((totalNo_Show_Physician + totalCanceled_Physician) / (totalArrived_Physician + totalNo_Show_Physician + totalCanceled_Physician)) - ((totalNo_Show_Physician_prior_year + totalCanceled_Physician_prior_year) / (totalArrived_Physician_prior_year + totalNo_Show_Physician_prior_year + totalCanceled_Physician_prior_year)) || '-'
      }"),
      style = JS("function(rowInfo) {
    ...
    perc_noShowPlus_Physician_actualized = colDef(
      ...
      #   let totalArrived_Physician_prior_year = 0
      #   let totalNo_Show_Physician_prior_year = 0
      #   let totalLate_Cancel_Physician_prior_year = 0
      #   let totalArrived_Physician = 0
      #   let totalNo_Show_Physician = 0
      ...
      #     totalArrived_Physician_prior_year += row['Arrived_Physician_prior_year']
      #     totalNo_Show_Physician_prior_year += row['No_Show_Physician_prior_year']
      #     totalLate_Cancel_Physician_prior_year += row['Late_Cancel_Physician_prior_year']
      #     totalArrived_Physician += row['Arrived_Physician']
      #     totalNo_Show_Physician += row['No_Show_Physician']
      #     totalLate_Cancel_Physician += row['Late_Cancel_Physician']
      #   })
      #   return Math.max(0,((((totalNo_Show_Physician_prior_year + totalLate_Cancel_Physician_prior_year) / (totalArrived_Physician_prior_year + totalNo_Show_Physician_prior_year + totalLate_Cancel_Physician_prior_year)) - ((totalNo_Show_Physician + totalLate_Cancel_Physician) / (totalArrived_Physician + totalNo_Show_Physician + totalLate_Cancel_Physician))) * (totalArrived_Physician + totalNo_Show_Physician + totalLate_Cancel_Physician))) || '-'
      # }"),
    ...
    perc_noShowPlus_Physician_actualized_dollar = colDef(
      ...
      #     totalLate_Cancel_Physician += row['Late_Cancel_Physician']
      #   })
      #   return Math.max(0,((((totalNo_Show_Physician_prior_year + totalLate_Cancel_Physician_prior_year) / (totalArrived_Physician_prior_year + totalNo_Show_Physician_prior_year + totalLate_Cancel_Physician_prior_year)) - ((totalNo_Show_Physician + totalLate_Cancel_Physician) / (totalArrived_Physician + totalNo_Show_Physician + totalLate_Cancel_Physician))) * (totalArrived_Physician + totalNo_Show_Physician + totalLate_Cancel_Physician))*110) || '-'
      # }"),
      style = list(fontWeight = "Bold", background = "#fcdcf0"),
    ...
    total_scheduled = colDef(
      name = "2025 YTD (Scheduled Volume)",
      maxWidth = 100,
      headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = "sum",
      format = colFormat(separators = TRUE, digits = 0)),
    
    total_scheduled_prior_year = colDef(
      name = "2024 Baseline (Scheduled Volume)",
      maxWidth = 100,
      headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = "sum",
      format = colFormat(separators = TRUE, digits = 0)),
    
    ref_converted_mshs_target = colDef(
      name = "2025 Target (Scheduled Volume)",
      maxWidth = 100,
      headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = "sum",
      format = colFormat(separators = TRUE, digits = 0)),
    ...
    ref_converted_var_target = colDef(
      name = "Variance to 2025 Target (Volume)",
      maxWidth = 100,
      headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = "sum",
      style = JS("function(rowInfo) {
    ...
    perc_conversion_prior_year = colDef(
      name = "2024 Baseline (Conversion Rate)",
      maxWidth = 100,
      headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
    ...
    perc_conversion_var_baseline = colDef(
      name = "Variance to 2024 (Conversion Rate)",
      maxWidth = 100,
      headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totaltotal_referrals = 0
        ...
        return ((totaltotal_scheduled) / (totaltotal_referrals)) - ((totaltotal_scheduled_prior_year) / (totaltotal_referrals_prior_year)) || '-'
      }"),
      style = JS("function(rowInfo) {
    ...
    avail_hour_adj_prior_year = colDef(show =F),
    ...
    perc_noShowPlus_prior_year = colDef(show =F),
    No_Show_Physician_prior_year = colDef(show =F),
    Canceled_Physician_prior_year = colDef(show =F),
    perc_noShow_Physician_prior_year = colDef(show =F),
    perc_SameDayCanceled_Physician_prior_year = colDef(show =F),
    
    perc_conversion_mshs_target = colDef(show =F),
    ...
    perc_noShowPlusPlus_prior_year = colDef(show =F),
    perc_noShowPlusPlus_Physician_prior_year = colDef(show =F),
    Rescheduled_Physician_prior_year = colDef(show =F),
    
    Canceled_prior_year = colDef(show =F),
    Rescheduled_prior_year = colDef(show =F)
