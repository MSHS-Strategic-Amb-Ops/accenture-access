...
reactable(
  ...
  columnGroups = list(
    colGroup(name = "", columns = c("Total", "Year", "Month"),
             ...
             sticky = "left"),
    colGroup(name = paste0("Volume Completed by Physicians*"),
    ...
    colGroup(name = paste0("C.1.1 No Show & Late Cancel Reduction*"),
...
```
<br/>
<p style="font-size:10pt; font-style:italic">*Volume, Provider Utilization, and No Show metrics shown on this page are based on physician visits only (PROV_TYPE = "Physician"); Template Utilization excludes providers with monthly provider utilization <35% or >200%.<p/>
<p style="font-size:10pt; font-style:italic">**Additional Visits Completed based on sum of monthly total, not YTD calculated.<p/>
<p style="font-size:10pt; font-style:italic">***Estimated Revenue Impact is calculated based on (2025 - 2024 No Show Plus Rate) X (Arrived + No Show + Late Canceled) X ($110 per Visit); sum of monthly estimated revenue impact, not YTD calculated.<p/><br/>
<br/>

## Metrics by Department
```{r New Patients Scheduled within 14 Days Output, echo = FALSE, warning = FALSE, message = FALSE}
...
htmltools::browsable(
  tagList(
    tags$button(
      tagList(fontawesome::fa("download"), "Download Table"),
      onclick = "Reactable.downloadDataCSV('access-metrics', 'Metrics by Department')"
    ),
...
reactable(
  ...
  columns = list(
    ...
    perc_new_los = colDef(
      name = "Physician New Patient Ratio*",
      maxWidth = 100,
      headerStyle = list(background = "#9E1A6D", color = "white", fontWeight = "Bold", fontSize = "14px"),
      # Calculate the aggregate conversion rate as `(total-pending) / total`
      aggregate = JS("function(values, rows) {
...
```
<br/>
<p style="font-size:10pt; font-style:italic">*New Patient is defined by level of service; Physician New Patient Ratio is calculated based on (Total New Patients Arrived for Physicians) / (Total Patients Arrived for Physicians)<p/>
