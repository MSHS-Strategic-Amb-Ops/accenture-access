...
sched_cnt_tbl <- tbl(conn1, "SCHED_CNT")
...
no_show_prov <- access_raw_tbl %>%
  ...
  filter(CONTACT_DATE < TO_DATE(date_start, "YYYY-MM-DD HH24:MI:SS")) %>%
  # dplyr::select(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_NAME_WID, PAT_ENC_CSN_ID, APPT_STATUS_NAME, CANCEL_LEAD_HOURS, CANCEL_INITIATOR) %>%
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, FINANCIAL_DIVISION, appt_year, appt_month, PROV_ID, NPI, PROV_NAME_WID, PROV_TYPE, DERIVED_STATUS_DESC) %>%
  summarise(total = n()) %>%
  collect() %>%
  pivot_wider(names_from = DERIVED_STATUS_DESC,
              values_from = total,
...
no_show_prov_physician <- access_raw_tbl %>%
  left_join(clarity_ser_tbl %>% select(PROV_ID, PROV_TYPE)) %>%
  filter(PROV_TYPE == "Physician") %>%
  left_join(clarity_ser_2_tbl %>% 
              dplyr::select(PROV_ID, NPI)) %>%
...
new_pt_ratio_prov <- access_raw_tbl %>%
  ...
  filter(CONTACT_DATE < TO_DATE(date_start, "YYYY-MM-DD HH24:MI:SS")) %>%
  filter(DERIVED_STATUS_DESC %in% c("Arrived")) %>%
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, FINANCIAL_DIVISION, appt_year, appt_month, PROV_ID, NPI, PROV_NAME_WID, PROV_TYPE, new_pt_los) %>%
  summarise(total = n()) %>%
  collect() %>%
  pivot_wider(names_from = new_pt_los,
              values_from = total,
...
new_pt_waitTime_prov <- access_raw_tbl %>%
  ...
  filter(APPT_MADE_DATE < TO_DATE(date_start, "YYYY-MM-DD HH24:MI:SS")) %>%
  filter(visit_type == "NEW") %>%
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, FINANCIAL_DIVISION, PROV_ID, NPI, PROV_NAME_WID, PROV_TYPE, appt_made_year, appt_made_month, visit_type, lead_14days) %>%
  summarise(total = n()) %>%
  collect() %>%
  pivot_wider(names_from = c("visit_type", "lead_14days"),
              values_from = total,
...
new_pt_waitTime_prov_payer <- access_raw_tbl %>%
  ...
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, FINANCIAL_DIVISION, PROV_ID, NPI, PROV_NAME_WID, PROV_TYPE, appt_made_year, appt_made_month, visit_type, FINCLASS, lead_14days) %>%
  summarise(total = n()) %>%
  collect() %>%
  pivot_wider(names_from = c("visit_type", "FINCLASS", "lead_14days"),
              values_from = total,
...
payer_mix_prov <- access_raw_tbl %>%
  ...
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, FINANCIAL_DIVISION, appt_year, appt_month, PROV_ID, NPI, PROV_NAME_WID, PROV_TYPE, new_pt_los, FINCLASS) %>%
  summarise(total = n()) %>%
  collect() %>%
  pivot_wider(names_from = c("new_pt_los","FINCLASS"),
              values_from = total,
...
appt_avail_online_prov <- appt_online %>%
  filter(avail_online_exclude == "N") %>%
  group_by(SITE, SPECIALTY_GRP, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, FINANCIAL_DIVISION, appt_made_year, appt_made_month, PROV_ID, NPI, PROV_NAME_WID, PROV_TYPE, avail_online, sched_online) %>%
  summarise(appts_sched = sum(total, na.rm = T))  %>%
  pivot_wider(names_from = c("avail_online", "sched_online"),
              values_from = appts_sched,
              values_fill = 0) %>%
...
require(openxlsx)
write.xlsx(prov_merged, "prov_merged_081925.xlsx")
...
reactable(
  ...
  columns = list(
    ...
    sched_hour_prior_year = colDef(show =F),
